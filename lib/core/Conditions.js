...
export default class Conditions{
  ...
  getMatchedCondition(){
    ...
    const candidates = conditions.filter( (condition) => {
      ...
      const allMatched = ![ targetMatched, ...extraKeysMatchedArray ].includes(false);
      return allMatched;
    });
    ...
    const scores = candidates.map( (condition) => {
      ...
      const targetMatched = this.targetMatcher( false, condition, this.context.latestInput );
      const extraKeysMatchedArray = this.extraSlotKeys.map( (key) => {
        return this.additionalInfoMatcher( false, condition, key, this.context.latestInput );
      });

      // マッチした数を数えてそれをスコアとする
      const s = { score: countTrueValue([ targetMatched, ...extraKeysMatchedArray ]) };
      return { 
        ...condition, ...s,
      };
    });
    ...
    return scores.filter( s => s.score === maxScore )[0] || { actionId: DEFAULT_RESPONSE_ACTION_ID };
  }
  ...
  targetMatcher(allowIgnore, condition, context ){
    if( isIgnoreOperator( condition.target ) ){ return !!allowIgnore; }
    if( !condition.target ){ return true; }
    if( !context.body.target ){ return false; }
  ...
  additionalInfoMatcher( allowIgnoreAndDefault, condition, bodyKey, context ){
    ...
    const bodyHasKey = !!context.body[bodyKey];
    ...
    const matchedWildCard = conditionIsWildCard && bodyHasKey;
    // Unfilled `?` は
    // 1) 入力が空きだったときにマッチする
    // 2) もしくは入力が `?` で条件がなにかあるときにマッチする
    const matchedUnfilled = (conditionIsUnfilledSlot && !bodyHasKey || contextIsUnfilledSlot && !!condition[bodyKey]);
  ...
  getTopicIdToTake(){
    ...
    if( consensusTopic && consensusTopic.id ){ return consensusTopic.id; }
    ...
    const contextTopicUsesConsensusValue = keys.map( (key) => {
    ...
    if( !contextTopicUsesConsensusValue ){ return IGNORE_OPERATOR; }
...
export function conditionUses( condition, key ){
  if( !condition || !condition[key] ){ return false; }
