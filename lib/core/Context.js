...
export default class Context{
  ...
  constructor( userId, redisPool, config ){
    ...
    this.body = new ContextSlots( { extraSlotKeys, initialLifeSpan, holdUsedSlot }, {});
  }
  createNewContextSlots(newInput){
    return new ContextSlots( { extraSlotKeys: this.extraSlotKeys, initialLifeSpan: this.initialLifeSpan }, newInput );
  };
  ...
  serialize(){
    ...
    return JSON.stringify( { userId, latestInput, matchedCondition, body, extra } );
  }

  persist(){
    return this.redisPool.setAsync( this.userId, this.serialize() )
      .catch( (e)=>{
   ...
   */
  clear() {
    return this.redisPool.delAsync( this.userId )
      .catch( (e)=>{
  ...
  static getOrInitial( userId, redisPool, options ){
    ...
    return redisPool.getAsync( userId ).then( (r) => {
      if( !r ){
        ...
        return new Context( userId, redisPool, { extraSlotKeys, initialLifeSpan, holdUsedSlot });
      }

      const context = new Context( userId, redisPool, { extraSlotKeys, initialLifeSpan, holdUsedSlot });
